...
public class JoinServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		// 위 방식(/* */ 안에 있는 코드)으로 보여줄 필요가 없다면 바로 폼을 보여주면된다.
	} // GET 방식으로 들어올경우 허용하지않는다.

	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		response.setContentType("text/html;charset=UTF-8"); // 한글은 항상 인코딩 고려해야함
		...
		// 파라미터 받기 전에 반드시 한글은 인코딩 설정 해줘야한다.
