	...
	<ul>
		...
		<c:forEach items="${guestList}" var="guest" varStatus="status">
			<c:if test="${status.first}">
				<li><b>[${guest.guest_no}]:${guest.guest_name}:${guest.guest_email}${status}</b></li>
			</c:if>
			<c:if test="${!status.first}">
				<li>[${guest.guest_no}]:${guest.guest_name}:${guest.guest_email}${status}</li>
			</c:if>
		</c:forEach>
		<br>
		<c:forEach items="${guestList}" var="guest" varStatus="status">
			<c:if test="${status.index%2==0}">
				<li><b>[${guest.guest_no}]:${guest.guest_name}${status}</b></li>
			</c:if>
			<c:if test="${status.index%2==1}">
				<li>[${guest.guest_no}]:${guest.guest_name}${status}</li>
			</c:if>
		</c:forEach>
		...
		<c:forEach items="${guestMap}" var="entry">
			<code>
				&it;c:set var="guest" scope="page" value="&dollar;{entry.value}" /&gt; <br>
				pageContext.setAttribute("guest", entry.value)
			</code>
			<c:set var="guest" scope="page" value="${entry.value}" />
			<li>${entry.key} ==> [${entry.value.guest_no} ${entry.value.guest_name}]</li>
			<li>[${guest.guest_no}]${guest.guest_name}${guest.guest_homepage}</li>
		</c:forEach>
		<br>
