0<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@taglib prefix="itwill" uri="http://www.itwill.co.kr/simpleTag" %>
<%
	session.setAttribute("sUserId", "guard");
	request.setAttribute("name", "현빈");
%>   
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body >
<h1>custom tag[사용자정의태그]</h1><hr/>
---------------hello tag----------------------<br>
<itwill:hello/>
<itwill:hello/>

---------------helloAttr tag----------------------<br>
<itwill:helloAttr irum="현빈"/>
<itwill:helloAttr irum="홍길동"/>
<itwill:helloAttr irum="${name}"/> <!-- "irum" does not support runtime expressions -->
---------------if tag[body]----------------------<br>
<itwill:if test="${empty sUserId == null}">
	<a href="user_login_form.jsp">로그인</a>
</itwill:if>
<itwill:if test="${empty sUserId != null}">
	<a href="user_logout_action.jsp">${sUserId} 님 로그아웃</a>
</itwill:if>


</body>
</html>