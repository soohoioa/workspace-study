...
SELECT * FROM ( SELECT rownum AS rn, sorted_board. * 
FROM (SELECT * FROM board ORDER BY groupno DESC, step ASC) sorted_board ) WHERE rn >= 71 AND rn <=80;
...
--게시판리스트 페이지
SELECT * 
FROM(SELECT rownum idx, sorted_board.* 
     FROM(SELECT boardno, title,writer,regdate, readcount,groupno, step, depth 
          FROM board 
          ORDER BY groupno desc,step asc) sorted_board) rownum_board
