...
/*
세션장바구니객체에 count라는 key로 Integer객체(덩연히 객체만 저장된다.)를 저장합니다.
세션장바구니객체는 저장데이타를 HashMap으로 관리합니다.
*/
// HttpSession session = request.getSession(true); 이 작업을 할 필요가 없다 : 알아서 해줌.
int count = 0; // 기본형데이터

Integer countInt =  (Integer)session.getAttribute("count");
if(countInt == null) { // 꺼낸데이터; 일단 꺼내봐야 안다. 있으면 꺼내서 하나 더하고 다시 넣는다.
	// count key 속성 객체가 존재하지 않는경우; 최초요청이라기 보다는 카운터가 아직 저장안된 상태.
	count = 0;
	
} else {
	// count key 속성 객체가 존재하는 경우;
	count = countInt.intValue();//기본형 데이터는 넣을수없다;
